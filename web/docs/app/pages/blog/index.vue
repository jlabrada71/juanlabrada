<template>
  ...
  <script setup>
    ...
    const query = ref('');
    const documentQuery = ref({ tag: "", filter: "" });
    ...
    async function findArticlesAll() {
    ...
    async function findArticles( {tag="", filter=""}, options = { skip: 0, limit: 10 }) {
      ...
      const results = await queryCollection('blog')
        ...
        .limit(limit)
        .where("tags", "LIKE", `%"${tag}"%`)
        // .andWhere("description", "LIKE", `%"${filter}"%`)
        .all()
      // console.log(`query: ${JSON.stringify(where)} skip: ${skip} toSkip: ${toSkip} limit: ${limit} Result length: ${results.length}`);
    ...
    async function loadMore() {
      const result = await findArticles(documentQuery.value, { skip: toSkip, limit: toLimit });
    ...
    watch([query, selectedCategory], async ([newQuery, newCategory], [oldQuery, oldCategory]) => {
      ...
      documentQuery.value = { tag: newCategory, filter: newQuery };
    ...
    onMounted(async () => {
      ...
      const result = await findArticlesAll();
