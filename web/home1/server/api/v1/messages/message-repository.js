...
export default class MessageRepository {
  ...
  async insert (record) {
    debug('Inserting new record', 'message-repository')
    debug(record)
    record.insertDate = Date.now()
    let client
    let result
    try {
  ...
  async select (record) {
    debug('Selecting ' + record._id, 'message-repository')
    let client
    let result = []
    try {
      client = await MongoClient.connect(this.connection /* process.env.MONGO_URL */, {
  ...
  async update (record, newValues) {
    debug('Updating ' + record._id, 'message-repository')
    newValues.updateDate = Date.now()
    let client
    let result
    try {
      client = await MongoClient.connect(this.connection /* process.env.MONGO_URL */, {
  ...
  async deleteRecord (record) {
    debug('Deleting ' + record._id, 'message-repository')
    let client
    let result
    try {
      client = await MongoClient.connect(this.connection /* process.env.MONGO_URL */, {
