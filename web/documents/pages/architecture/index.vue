<template>
      ...
      <div>
        ...
        <div class="flex flex-wrap">
          <div v-for="category in categories" :key="category">
            <div v-if="selectedCategory != category" class="badge badge-neutral badge-xs cursor-pointer" @click="selectedCategory = category">{{ category }}</div>
            <div v-else class="badge badge-accent badge-xs cursor-pointer" @click="selectCategory">{{ category }}</div>
          </div>
        </div>
          ...
          <!-- <div v-if="articles.length"> -->
            ...
            <ul >
              <li v-for="article of articles" :key="article.slug" >
                <NuxtLink class="text-xs" :to="`${article.slug}`" >{{ article.title }}</NuxtLink>
              </li>
            </ul>
  ...
  <script setup>
    ...
    async function findArticles( where = {}, options = { skip: 0, limit: 10 }) {
      ...
      const results = await queryContent('architecture')
