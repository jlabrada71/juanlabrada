...
export default class AnalyticsRepository {
  ...
  async insert (record) {
    debug('Inserting new analytics', 'analytics-repository')
    debug(record)
    let client
    let result
    try {
  ...
  async select (record) {
    debug('Selecting ' + record._id, 'analytics-repository')
    let client
    let result = []
    try {
      client = await MongoClient.connect(this.connection /* process.env.MONGO_URL */, {
  ...
  async update (record, newValues) {
    debug('Updating ' + record._id, 'analytics-repository')
    let client
    let result
    try {
      client = await MongoClient.connect(this.connection /* process.env.MONGO_URL */, {
  ...
  async deleteRecord (record) {
    debug('Deleting ' + record._id, 'analytics-repository')
    let client
    let result
    try {
      client = await MongoClient.connect(this.connection /* process.env.MONGO_URL */, {
