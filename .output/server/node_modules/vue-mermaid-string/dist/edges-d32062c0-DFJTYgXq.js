import { p as H, c as b, d as q, b7 as Q, h as E, l as g, A as j, F as lt } from "./entry-B7RwYlCB.js";
import { a as st } from "./createText-6b48ae7d-CpnWlYZ_.js";
import { l as ct } from "./line-NK_0ZW6o.js";
const ht = (e, t, a, i) => {
  t.forEach((l) => {
    wt[l](e, a, i);
  });
          ...
          (u) => new Promise((S) => {
            function B() {
              if (u.style.display = "flex", u.style.flexDirection = "column", x) {
                const C = b().fontSize ? b().fontSize : window.getComputedStyle(document.body).fontSize, A = parseInt(C, 10) * 5 + "px";
...
const Ft = (e, t) => {
  ...
  t.classData.members.forEach((v) => {
    ...
    let $ = N.getBBox();
    if (H(b().flowchart.htmlLabels)) {
      const G = N.children[0], D = E(N);
      $ = G.getBoundingClientRect(), D.attr("width", $.width), D.attr("height", $.height);
    }
  ...
  }), p += l, o.attr("class", "divider").attr("x1", -h / 2 - a).attr("x2", h / 2 + a).attr("y1", -y / 2 - a + l + p).attr("y2", -y / 2 - a + l + p), p += l, A.forEach((v) => {
    E(v).attr(
      "transform",
