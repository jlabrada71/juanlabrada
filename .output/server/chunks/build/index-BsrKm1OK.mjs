import { _ as __nuxt_component_0$5 } from './nuxt-link-C85iBeMR.mjs';
import { mergeProps, withCtx, createTextVNode, toDisplayString, ref, computed, isRef, unref, createVNode, withModifiers, nextTick, defineComponent, shallowRef, getCurrentInstance, provide, cloneVNode, h, createElementBlock, useSSRContext } from 'vue';
...
import { _ as _sfc_main$e, A as ApiRequest, l as log, g as gsapWithCSS } from './api-request-CzZAkO0p.mjs';
...
const _sfc_main$c = {
  ...
  setup(__props) {
    ...
    async function openModal() {
      ...
      await nextTick();
      gsapWithCSS.set(overlay.value, { opacity: 0 });
      gsapWithCSS.set(modal.value, { opacity: 0, scale: 0.9, y: 24 });
      gsapWithCSS.to(overlay.value, { opacity: 1, duration: 0.25, ease: "power2.out" });
      gsapWithCSS.to(modal.value, { opacity: 1, scale: 1, y: 0, duration: 0.35, ease: "back.out(1.6)" });
      (void 0).addEventListener("keydown", onKeydown);
      (void 0).body.style.overflow = "hidden";
    }
    function closeModal() {
      if (!photoModalOpen.value) return;
      gsapWithCSS.to(modal.value, { opacity: 0, scale: 0.9, y: 24, duration: 0.2, ease: "power2.in" });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--[--><header class="fixed top-0 left-0 right-0 z-50 h-16 flex items-center justify-between px-8" style="${ssrRenderStyle({ "background": "rgba(11,19,38,0.8)", "backdrop-filter": "blur(24px)", "-webkit-backdrop-filter": "blur(24px)", "border-bottom": "1px solid rgba(0,240,255,0.1)" })}">`);
      _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(` Juan Labrada <img${ssrRenderAttr("src", _imports_0$1)} alt="Juan Labrada" class="w-8 h-8 rounded-full object-cover cursor-pointer" style="${ssrRenderStyle({ "border": "1px solid rgba(0, 240, 255, 0.3)" })}"${_scopeId}>`);
          } else {
            return [
              createTextVNode(" Juan Labrada "),
              createVNode("img", {
                src: _imports_0$1,
                alt: "Juan Labrada",
      ...
      if (mobileOpen.value) {
        _push(`<div class="absolute top-16 left-0 right-0 flex flex-col items-start gap-4 p-6" style="${ssrRenderStyle({ "background": "rgba(11,19,38,0.96)", "backdrop-filter": "blur(24px)", "border-bottom": "1px solid rgba(0,240,255,0.1)" })}"><!--[-->`);
        ssrRenderList(navLinks, (link) => {
          _push(`<a${ssrRenderAttr("href", link.href)} class="label-caps text-cyber-muted hover:text-cyber-text transition-colors">${ssrInterpolate(link.label)}</a>`);
        });
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (photoModalOpen.value) {
          _push2(`<div class="fixed inset-0 z-[100] flex items-center justify-center p-6" style="${ssrRenderStyle({ "background": "rgba(6, 14, 32, 0.8)", "backdrop-filter": "blur(4px)" })}"><div class="glass-card relative max-w-md w-full max-h-[85vh] overflow-y-auto p-6 flex flex-col items-center gap-2"><button type="button" aria-label="Close" class="absolute top-3 right-3 w-10 h-10 rounded-full glass-card flex items-center justify-center text-cyber-primary hover:shadow-glow-primary transition-shadow"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg></button><img${ssrRenderAttr("src", _imports_0$1)} alt="Juan Labrada" class="w-32 h-32 rounded-full border border-cyber-primary/30 shadow-glow-primary object-cover mt-2"><span class="label-caps text-cyber-text text-center">Juan Labrada</span>`);
...
const _sfc_main$b = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (activeCert.value) {
          _push2(`<div class="fixed inset-0 z-[100] flex items-center justify-center p-6" style="${ssrRenderStyle({ "background": "rgba(6, 14, 32, 0.8)", "backdrop-filter": "blur(4px)" })}" data-v-186827be><div class="glass-card relative max-w-3xl w-full p-6 flex flex-col gap-4" data-v-186827be><button type="button" aria-label="Close" class="absolute top-3 right-3 w-10 h-10 rounded-full glass-card flex items-center justify-center text-cyber-primary hover:shadow-glow-primary transition-shadow" data-v-186827be><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor" data-v-186827be><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" data-v-186827be></path></svg></button><img${ssrRenderAttr("src", `/certificates/${activeCert.value.file}`)}${ssrRenderAttr("alt", activeCert.value.title)} class="w-full max-h-[75vh] object-contain rounded-xl border border-cyber-primary/30 shadow-glow-primary" data-v-186827be><span class="label-caps text-cyber-text text-center" data-v-186827be>${ssrInterpolate(activeCert.value.title)}</span></div></div>`);
        } else {
          _push2(`<!---->`);
        }
      }, "body", false, _parent);
...
function _sfc_ssrRender$5(_ctx, _push, _parent, _attrs) {
  ...
  }, _attrs))}><div class="absolute inset-0 pointer-events-none" style="${ssrRenderStyle({ "background-image": "linear-gradient(rgba(0,240,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(0,240,255,0.03) 1px, transparent 1px)", "background-size": "48px 48px" })}"></div><div class="relative z-10 flex flex-col items-center text-center gap-6 max-w-4xl mx-auto w-full min-w-0"><span class="label-caps text-cyber-primary"> Fullstack AI Engineer · Claude · LangChain · Vue.js · Node.js </span><h1 class="font-geist font-bold text-cyber-text leading-tight" style="${ssrRenderStyle({ "font-size": "clamp(2.5rem, 6vw, 4rem)", "letter-spacing": "-0.02em" })}"> Solving the <span style="${ssrRenderStyle({ "color": "#00f0ff" })}">unsolvable</span><span class="text-cyber-text"> with AI-driven architecture</span></h1><p class="text-cyber-text">Specializing in AI Solutions with Vue.js/Nuxt frontend systems and highly scalable Node.js backend infrastructures.</p>`);
...
const _sfc_main$9 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))}><div class="max-w-5xl mx-auto"><p class="label-caps text-cyber-primary mb-3">Career</p><h2 class="font-geist font-semibold text-cyber-text mb-12" style="${ssrRenderStyle({ "font-size": "2.5rem", "letter-spacing": "-0.01em" })}"> Experience </h2><div class="flex flex-col gap-6"><!--[-->`);
...
const _sfc_main$8 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))}><div class="max-w-5xl mx-auto"><p class="label-caps text-cyber-primary mb-3">Work</p><h2 class="font-geist font-semibold text-cyber-text mb-12" style="${ssrRenderStyle({ "font-size": "2.5rem", "letter-spacing": "-0.01em" })}"> Products </h2><div class="grid grid-cols-1 md:grid-cols-3 gap-6"><!--[-->`);
        ...
        _push(ssrRenderComponent(_component_NuxtLink, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              return [
                createTextVNode(toDisplayString(product.linkLabel), 1)
              ];
            }
          }),
          _: 2
...
function _sfc_ssrRender$4(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${ssrRenderAttrs(mergeProps({ id: "card" }, _attrs))} data-v-dbdbae5a>`);
...
const _sfc_main$4 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(unref(VueMermaidString), { value: diagram2.value }, null, _parent));
...
function _sfc_ssrRender$1(_ctx, _push, _parent, _attrs) {
  ...
  _push(`<section${ssrRenderAttrs(mergeProps({
    ...
    class: "w-full py-24 px-6",
    style: { "background": "#0b1326" }
  }, _attrs))}><div class="max-w-5xl mx-auto"><p class="label-caps text-cyber-primary mb-3">Playground</p><h2 class="font-geist font-semibold text-cyber-text mb-12" style="${ssrRenderStyle({ "font-size": "2.5rem", "letter-spacing": "-0.01em" })}"> Experiments </h2><div class="grid grid-cols-1 md:grid-cols-3 gap-6"><div class="glass-card p-6 flex flex-col gap-3"><h3 class="font-geist font-semibold text-cyber-text">Gallery 1</h3><p class="font-hanken text-cyber-muted text-sm">Changing image width and height on hover within a grid.</p><div class="mt-auto overflow-hidden rounded-lg">`);
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<section${ssrRenderAttrs(mergeProps({
        ...
        class: "w-full py-24 px-6",
        style: { "background": "#0d1628" }
      }, _attrs))}><div class="max-w-5xl mx-auto"><p class="label-caps text-cyber-primary mb-3">Writing</p><h2 class="font-geist font-semibold text-cyber-text mb-12" style="${ssrRenderStyle({ "font-size": "2.5rem", "letter-spacing": "-0.01em" })}"> Posts </h2><div class="grid grid-cols-1 md:grid-cols-3 gap-6"><!--[-->`);
      ssrRenderList(posts, (post) => {
        _push(`<a${ssrRenderAttr("href", post.url)} target="_blank" rel="noopener" class="glass-card p-6 flex flex-col gap-3 group" style="${ssrRenderStyle({ "text-decoration": "none" })}"><h3 class="font-geist font-semibold text-cyber-text group-hover:text-cyber-primary transition-colors">${ssrInterpolate(post.title)}</h3><span class="label-caps text-cyber-primary mt-auto">Read →</span></a>`);
      });
      _push(`<!--]--></div></div></section>`);
    };
  }
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    handleSubmit(async (values2, actions) => {
      ...
      await messageRepository.add({ ...values2, date: new Date(Date.now()) });
      loading.value = false;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<section${ssrRenderAttrs(mergeProps({
        ...
        class: "w-full py-24 px-6",
        style: { "background": "#0b1326" }
      }, _attrs))}><div class="max-w-2xl mx-auto"><p class="label-caps text-cyber-primary mb-3">Let&#39;s Talk</p><h2 class="font-geist font-semibold text-cyber-text mb-12" style="${ssrRenderStyle({ "font-size": "2.5rem", "letter-spacing": "-0.01em" })}"> Contact Me </h2>`);
      ...
      if (unref(errorList).length > 0) {
        _push(`<div class="p-3 rounded" style="${ssrRenderStyle({ "background": "rgba(255,180,171,0.1)", "border": "1px solid rgba(255,180,171,0.3)" })}"><ul class="text-sm" style="${ssrRenderStyle({ "color": "#ffb4ab" })}"><!--[-->`);
      ...
      }), mergeProps(_temp1, ssrGetDynamicModelProps(_temp1, unref(email)))))}></div><div class="flex flex-col gap-1"><label for="message" class="label-caps text-cyber-muted">Message</label><textarea${ssrRenderAttrs(mergeProps({ id: "message" }, unref(messageAttrs), {
        rows: "5",
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs) {
  ...
  _push(`<div${ssrRenderAttrs(mergeProps({ class: "bg-cyber-bg w-full" }, _attrs))}>`);
...
//# sourceMappingURL=index-BsrKm1OK.mjs.map
