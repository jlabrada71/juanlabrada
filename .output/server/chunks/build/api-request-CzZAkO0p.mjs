...
import { computed, ref, watch, nextTick, useSSRContext } from 'vue';
...
function log(message, context = "main", type = "LOG", callerDepth = 4) {
  ...
  formatDate(new Date(Date.now())) + ": " + message;
}
function debug(message, context) {
}

const _sfc_main = {
  __name: "ActionFeedback",
  ...
  setup(__props, { emit: __emit }) {
    ...
    function startTimer() {
      ...
      gsapWithCSS.set(progress.value, { scaleX: 1, transformOrigin: "left" });
      progressTween = gsapWithCSS.to(progress.value, {
        scaleX: 0,
        duration: props.timeout / 1e3,
        ease: "none",
        onComplete: close
      });
    }
    watch(() => props.show, async (isShown) => {
      ...
      await nextTick();
      gsapWithCSS.fromTo(toast.value, { opacity: 0, y: 16, scale: 0.96 }, { opacity: 1, y: 0, scale: 1, duration: 0.35, ease: "back.out(1.6)" });
      startTimer();
    }, { immediate: true });
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        if (props.show) {
          _push2(`<div class="fixed bottom-6 right-6 z-[100] w-full max-w-sm glass-card overflow-hidden" role="status"><div class="flex items-start gap-3 p-4"><span class="flex-none w-9 h-9 rounded-full flex items-center justify-center" style="${ssrRenderStyle({ background: accent.value.bg, color: accent.value.fg })}">`);
          if (color.value === "error") {
            _push2(`<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l6.518 11.59c.75 1.334-.213 2.98-1.742 2.98H3.48c-1.53 0-2.492-1.646-1.743-2.98l6.52-11.59zM11 14a1 1 0 11-2 0 1 1 0 012 0zm-.25-6.25a.75.75 0 00-1.5 0v3.5a.75.75 0 001.5 0v-3.5z" clip-rule="evenodd"></path></svg>`);
          } else {
            _push2(`<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd"></path></svg>`);
          }
          _push2(`</span><div class="flex-1 pt-1"><p class="label-caps" style="${ssrRenderStyle({ color: accent.value.fg })}">${ssrInterpolate(props.title)}</p><p class="font-hanken text-cyber-text text-sm mt-1">${ssrInterpolate(props.message)}</p></div><button type="button" aria-label="Dismiss" class="flex-none text-cyber-muted hover:text-cyber-text transition-colors"><svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg></button></div><div class="h-0.5 w-full bg-cyber-outline-subtle/40"><div class="h-full" style="${ssrRenderStyle({ background: accent.value.fg })}"></div></div></div>`);
        } else {
          _push2(`<!---->`);
        }
      }, "body", false, _parent);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/ActionFeedback.vue");
...
class ApiRequest {
  ...
  static request(config) {
    debug(`request API called: ${config.url}`);
...
export { ApiRequest as A, _sfc_main as _, gsapWithCSS as g, log as l };
//# sourceMappingURL=api-request-CzZAkO0p.mjs.map
